...
def main():
    ...
    if not st.session_state["logged_in"]:
        ...
        with c1:
            st.markdown(f"<h5 style='margin:0; white-space:nowrap;'>✅系統版本：20260128，歡迎回來，{st.session_state['user']}！</h5>", unsafe_allow_html=True)
        with c2:
...
# ==========================================
    ...
    # 乖離與均線診斷
    if price > ma20_v and bias_v > 8:
        diag_details.append("🏃 **衝太快了**：股價短期噴發過猛，乖離偏大，現在衝進去很容易被割在短線高點。")
    elif price < ma20_v:
        diag_details.append("🌧️ **還在淋雨**：股價掉在月線下方，上方全是套牢的人，在大勢轉強前，操作難度很高。")

    # 量價與法人資金診斷
    if m_val > 3 and vol_p < 80:
        diag_details.append("🧐 **小心誘多**：雖然股價在漲，但成交量明顯萎縮，這通常是主力『拉高出貨』的假動作。")
    elif m_val < -2 and "過熱" in s_val:
        diag_details.append("🚨 **大戶在溜**：市場情緒看起來很High，但數據顯示大戶資金正加速撤離，小心變接盤俠。")
    elif m_val > 1 and "恐慌" in s_val:
        diag_details.append("👀 **黃金背離**：大家都在怕，但 AI 偵測到有聰明錢在低檔偷偷接盤，這是止跌的初步訊號。")
