...
# --- 1. 配置與 UI 視覺 (完整還原 CSS 排版以符合行數基準) ---
...
def perform_ai_engine(df, p_days, precision, trend_weight, v_comp):
    ...
    return pred_prices, adv, curr_p, open_p, prev_c, curr_v, change_pct, (res[0], " | ".join(reasons), res[1], pred_prices[0], pred_prices[0] + (std_val * 1.5), pred_prices[0] - (std_val * 1.5))

# --- 4. 圖表渲染 (展開所有圖表代碼以對齊行數) ---
def render_terminal(symbol, p_days, cp, tw_val, api_ttl, v_comp):
    ...
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['MA60'], name='MA60', line=dict(color='#FFAC33', width=2)), 1, 1)
    ...
    fig.add_trace(go.Bar(x=p_df.index, y=p_df['Hist'], name='MACD', marker_color=h_colors), 3, 1)
                ...
                if st.button("🗑️ 刪除目前選定股票"):
                    ...
                    for i, r in enumerate(all_rows):
                        if r[0] == st.session_state.user and r[1] == target:
                            ws_w.delete_rows(i + 1)
                            st.rerun()
            with m2:
                p_days = st.number_input("預測天數", 1, 30, 7)
                if st.session_state.user == "okdycrreoo":
                    st.markdown("### 🛠️ 管理員戰情室")
                    b1 = st.text_input("1. 權值標本-藍籌股基準：用於校準 AI 對市場地基穩定度的感知。 (推薦: 2330)", s_map.get('benchmark_1', '2330'))
                    b2 = st.text_input("2. 成長標本-高波動指標：學習識別異常爆發或動能切換。 (推薦: 2317)", s_map.get('benchmark_2', '2317'))
                    b3 = st.text_input("3. ETF 標本-資金流向：協助 AI 過濾雜訊判斷族群輪動。 (推薦: 0050)", s_map.get('benchmark_3', '0050'))
                    new_p = st.slider("系統靈敏度", 0, 100, cp)
                    new_tw = st.number_input("AI 趨勢權重", 0.5, 3.0, tw_val)
                    new_ttl = st.number_input("API 快取 (分)", 1, 10, api_ttl)
                    ...
                    new_v = st.slider(f"波動補償 (建議: {rec_v})", 0.1, 3.0, v_comp)
