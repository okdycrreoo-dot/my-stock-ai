...
# --- 1. 配置與 UI 視覺 (確保 CSS 權重與原本 290 行版一致) ---
...
def perform_ai_engine(df, p_days, precision, trend_weight, v_comp):
    ...
    # --- 核心：新增 20日命中率回測 ---
    hits = 0; test_range = 20; backtest_data = df.tail(test_range + 1)
    for i in range(len(backtest_data)-1):
        ...
        # 回測區間判定同步 v_comp
        e_range = (day_prev['Close'] * (1 - day_vol * v_comp), day_prev['Close'] * (1 + day_vol * v_comp))
        if e_range[0] <= day_curr['Close'] <= e_range[1]: hits += 1
    ...
    return pred_prices, adv, curr_p, open_p, prev_c, curr_v, change_pct, (res[0], " | ".join(reasons), res[1], next_close, next_close + std_val, next_close - std_val, win_rate)
...
def render_terminal(symbol, p_days, cp, tw_val, api_ttl, v_comp):
    ...
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['MA5'], name='MA5 均線', line=dict(color='#FFFF00', width=2)), 1, 1)
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['MA20'], name='MA20 均線', line=dict(color='#00F5FF', width=1.5)), 1, 1)
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['MA60'], name='MA60 均線', line=dict(color='#FFAC33', width=2)), 1, 1)
    ...
    fig.add_trace(go.Bar(x=p_df.index, y=p_df['Volume'], name='成交量能', marker_color=v_colors), 2, 1)
    fig.add_trace(go.Bar(x=p_df.index, y=p_df['Hist'], name='MACD 力道', marker_color=['#FF3131' if v >= 0 else '#00FF41' for v in p_df['Hist']]), 3, 1)
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['K'], name='K值 (藍)', line=dict(color='#00F5FF')), 4, 1)
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['D'], name='D值 (黃)', line=dict(color='#FFFF00')), 4, 1)
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['J'], name='J值 (紫)', line=dict(color='#E066FF')), 4, 1)
    ...
    st.markdown(f"""
        <div class='ai-advice-box'>
            ...
            <div style='margin-top:10px; border:1px dashed #00F5FF; padding:10px; border-radius:8px;'>
                <p style='margin:0; font-weight:800; color:#00F5FF;'>📊 20日實戰績效回測：</p>
                <span style='font-size:0.9rem;'>預測準確度 (區間命中率)：</span><span class='backtest-val'>{insight[6]:.1f}%</span>
            </div>
        </div>
    """, unsafe_allow_html=True)

# --- 5. 主程式 (完整恢復管理員戰情室中文細節) ---
def main():
        ...
        with st.expander("⚙️ 終端設定面板", expanded=True):
            ...
            with m2:
                ...
                if st.session_state.user == "okdycrreoo":
                    ...
                    b1 = st.text_input("1. 權值標本-藍籌股基準：影響長期走勢合理性。(推薦: 2330)", s_map.get('benchmark_1', '2330'))
                    b2 = st.text_input("2. 成長標本-高波動指標：提高對急漲急跌預警。(推薦: 2317)", s_map.get('benchmark_2', '2317'))
                    b3 = st.text_input("3. ETF 標本-市場資金流向：判斷整體族群輪動。(推薦: 0050)", s_map.get('benchmark_3', '0050'))
                    ...
                    _, _, rec_v = auto_fine_tune_engine(t_df, cp, tw_val, v_comp) if t_df is not None else (0, 0, 1.5)
                    new_v = st.slider(f"波動補償係數 - 當前個股 AI 建議值: {rec_v}", 0.1, 3.0, v_comp)
