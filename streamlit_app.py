...
def chapter_7_ai_committee_analysis(symbol, brain_row):
    ...
    # 所有人皆可啟動，移除權限檢查
    if st.button(f"🚀 啟動 {symbol} 全球視野對撞分析", key="ai_ultimate_v9", type="primary", use_container_width=True):
        status = st.empty()
        ...
        try:
            ...
            # --- 第一階段：三段式精準情報搜集 ---
            status.info(f"🔍 步驟 1: 正在辨識 {pure_symbol} 公司全名與核心業務...")
            with DDGS() as ddgs:
                # 1. 正名：確保搜尋時用「群創」而非「3481.TW」
                name_info = [n['title'] for n in ddgs.text(f"台股 {pure_symbol} 公司全名", max_results=1)]
                company_name = name_info[0] if name_info else pure_symbol
                
                # 2. 在地與全球題材主動感知 (自動涵蓋面板、GPU、衛星、運價、稀土等熱點)
                status.info(f"📡 步驟 2: 抓取「{company_name}」在地新聞與全球產業鏈熱點...")
                tw_news = [n['body'] for n in ddgs.news(f"{company_name} 股價 新聞 報價 減資", max_results=5)]
                
                # 3. 強制提取：台指期夜盤具體點數與美股連動
                status.info(f"📊 步驟 3: 強制提取台指期夜盤數據與跨市場情報...")
                futures_data = [n['body'] for n in ddgs.text("台指期 夜盤 最新 漲跌點數", max_results=3)]
                us_adr_data = [n['body'] for n in ddgs.news(f"{company_name} ADR 美股 走勢", max_results=2)]

                all_context = "\n".join(tw_news + futures_data + us_adr_data)

            # --- 第二階段：Groq 極速對撞分析 ---
            status.info("⚖️ 正在對撞 40+ 指標並產出「實戰操作建議」...")
            ...
            # 強力 Prompt 指令，拒絕廢話，強制給出結論
            prompt = f"""
            你現在是頂尖實戰操盤手，拒絕任何空話。
            分析目標：{symbol} ({company_name})
            ...
            【分析指令 - 必須包含以下硬核內容】：
            1. 具體數據提取：必須從情報中指出「台指期夜盤」最新的漲跌點數，並直接說明對明日開盤的具體加減分。
            2. 供應鏈題材對撞：自動識別 {company_name} 的產業定位（如面板、AI、衛星等）。分析新聞中的具體利多（如報價漲幅、大廠訂單）是否反映在量化指標中。
            3. 陷阱警示：如果消息面極熱，但量化數據顯示乖離過大或法人出貨，必須明確指出「追高風險」。
            4. 週期判斷：此行情是受「夜盤影響」的短線波動，還是受「產業循環」帶動的長線趨勢？
            
            【5. 最終實戰結論 - 必須明確】：
            ■ 綜合評等：(強烈買入 / 觀望 / 避險 / 停損出清)
            ■ 明日開盤建議：(例如：平盤下承接、開高不追、或立即減碼)
            ■ 具體點位參考：(根據數據給出建議的支撐與壓力範圍)
            ■ 停利/停損策略：(給出具體的邏輯或百分比建議)
            """

            payload = {
                "model": "llama-3.3-70b-versatile",
                "messages": [{"role": "user", "content": prompt}],
                "temperature": 0.1 # 嚴格根據數據回報，不胡亂發散
            }
            ...
            if response.status_code == 200:
                ...
                st.markdown(f"#### 🗨️ {symbol} 全球實戰對撞診斷報告")
                st.markdown(report_content)
                st.success("✅ 實戰診斷完成，請依策略執行。")
            else:
                st.error(f"❌ Groq 引擎回報錯誤：{response.status_code}")
