...
def perform_ai_engine(df, p_days, precision, trend_weight, v_comp):
    ...
    return pred_prices, adv, curr_p, open_p, prev_c, curr_v, change_pct, (res[0], " | ".join(reasons), res[1], pred_prices[0], pred_prices[0] + std_val, pred_prices[0] - std_val)
...
def render_terminal(symbol, p_days, cp, tw_val, api_ttl, v_comp):
    ...
    m_cols = st.columns(5); c_p = "#FF3131" if change_pct >= 0 else "#00FF41"; sign = "+" if change_pct >= 0 else ""
    ...
    fig.add_trace(go.Scatter(x=f_dates, y=pred_line, name='AI 預測路徑', line=dict(color='#FF3131', dash='dash')), 1, 1)
        ...
        with st.expander("⚙️ 終端設定面板", expanded=True):
            ...
            with m2:
                ...
                if st.session_state.user == "okdycrreoo":
                    ...
                    b1 = st.text_input("1. 權值藍籌股標本 (推薦: 2330)", s_map.get('benchmark_1', '2330'))
                    b2 = st.text_input("2. 高波動成長股標本 (推薦: 2317)", s_map.get('benchmark_2', '2317'))
                    b3 = st.text_input("3. 指數 ETF 標本 (推薦: 0050)", s_map.get('benchmark_3', '0050'))
                    new_p = st.slider("系統靈敏度", 0, 100, cp)
                    new_tw = st.number_input("趨勢權重", 0.5, 3.0, tw_val)
