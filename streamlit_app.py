...
st.markdown(f"""
    <link rel="manifest" href="./manifest.json?v=8.0">
    <link rel="apple-touch-icon" href="{ICON_URL}?v=8.0"><link rel="icon" type="image/png" href="{ICON_URL}?v=8.0">
    <style>
    ...
    .stButton>button {{ background-color: #00F5FF !important; color: #0E1117 !important; border-radius: 12px; font-weight: 900 !important; }}
...
def perform_ai_advice(df, precision):
    ...
    def get_p(ma, cycle):
        ...
        # 綜合 MACD/KDJ/成交量修正
        mod = 0
        if last['Hist'] > prev['Hist']: mod += 0.01 # 動能轉強
        if last['J'] < 20: mod -= 0.005 # KDJ超賣，買點更保守
        if last['Volume'] > last['V_MA5']: mod += 0.005 # 有量
...
def show_dashboard(symbol, p_days, precision):
    ...
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['MACD'], name='MACD線', line=dict(color='#FFFFFF', width=1)), 3, 1)
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['Signal'], name='訊號線', line=dict(color='#FFFF00', width=1)), 3, 1)
    
    # 4. KDJ (補回 K、D 線)
    fig.add_trace(go.Scatter(x=p_df.index, y=p_df['K'], name='K', line=dict(color='#FFFFFF')), 4, 1)
    ...
    if not st.session_state.user:
        ...
        with st.expander("⚙️ 終端功能面板", expanded=False):
            ...
            with m1:
                ...
                if st.button("確認新增股票") and ns: ws_w.append_row([st.session_state.user, ns.upper()]); st.rerun()
            with m2:
