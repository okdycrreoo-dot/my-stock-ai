...
def perform_ai_engine(df, p_days, precision, trend_weight, v_comp):
    ...
    return pred_prices, adv, curr_p, open_p, prev_c, curr_v, change_pct, (res[0], " | ".join(reasons), res[1], next_close, next_close + (std_val * 1.5), next_close - (std_val * 1.5), accuracy_val)
...
def render_terminal(symbol, p_days, cp, tw_val, api_ttl, v_comp, ws_p):
    ...
    m_cols = st.columns(5)
    # 將第三格替換為準確性，其餘維持
    metrics = [("當前價格", f"{curr_p:.2f}", c_p), ("今日漲跌", f"{sign}{change_pct:.2f}%", c_p), ("AI 預估準確性", f"{insight[6]:.1f}%", "#FFAC33"), ("昨日收盤", f"{prev_c:.2f}", "#FFFFFF"), ("今日成交", f"{curr_v:,}", "#FFFF00")]
    ...
    fig.add_trace(go.Bar(x=p_df.index, y=p_df['Volume'], name='成交量能', marker_color=['#FF3131' if p_df['Close'].iloc[i] >= p_df['Open'].iloc[i] else '#00FF41' for i in range(len(p_df))], legendgroup="2"), 2, 1)
    ...
    fig.update_layout(template="plotly_dark", height=880, xaxis_rangeslider_visible=False, showlegend=True, margin=dict(r=180, t=50, b=50), legend=dict(orientation="v", yanchor="top", y=1, xanchor="left", x=1.02, tracegroupgap=155))
        ...
        with st.expander("⚙️ 終端設定面板", expanded=True):
            ...
            with m1:
                ...
                if st.button("🗑️ 刪除目前選定股票"):
                    for idx, row in enumerate(ws_w.get_all_values()):
                        if row[0] == st.session_state.user and row[1] == target: ws_w.delete_rows(idx + 1); st.rerun()
            with m2:
                p_days = st.number_input("預測天數", 1, 30, 7)
                if st.session_state.user == "okdycrreoo":
                    st.markdown("### 🛠️ 管理員戰情室")
                    b1 = st.text_input("1. 權值標本-藍籌股基準：用於校準 AI 對市場穩定度的感知。 (推薦: 2330)", s_map.get('benchmark_1', '2330'))
                    b2 = st.text_input("2. 成長標本-高波動指標：提高對急漲急跌的預警能力。 (推薦: 2317)", s_map.get('benchmark_2', '2317'))
                    b3 = st.text_input("3. ETF 標本-市場資金流向：判斷整體族群的板塊輪動。 (推薦: 0050)", s_map.get('benchmark_3', '0050'))
                    ...
                    if st.button("💾 同步參數"):
                        ws_s.update_cell(2, 2, str(new_p)); ws_s.update_cell(3, 2, str(new_ttl)); ws_s.update_cell(4, 2, b1); ws_s.update_cell(5, 2, b2); ws_s.update_cell(6, 2, b3); ws_s.update_cell(7, 2, str(new_tw)); ws_s.update_cell(8, 2, str(new_v))
