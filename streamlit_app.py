...
def perform_ai_engine(df, p_days, precision, trend_weight, v_comp):
    ...
    # 標準差 std_val 現在會隨著 v_comp 大小而劇烈伸縮
    ...
    return pred_prices, adv, curr_p, open_p, prev_c, curr_v, change_pct, (res[0], " | ".join(reasons), res[1], next_close, next_close + std_val, next_close - std_val)
...
def render_terminal(symbol, p_days, cp, tw_val, api_ttl, v_comp):
    ...
    # 關鍵：將 Google Sheets 讀取的 v_comp 傳入引擎
    ...
    m_cols = st.columns(5); c_p = "#FF3131" if change_pct >= 0 else "#00FF41"
    ...
    st.markdown(f"""
        <div class='ai-advice-box'>
            ...
            <div style='background: #1C2128; padding: 12px; border-radius: 8px;'>
                <p style='color:#00F5FF; font-weight:bold;'>🔮 AI 統一展望：</p>
                <p style='font-size:1.3rem; color:#FFAC33; font-weight:900;'>預估隔日收盤：{insight[3]:.2f}</p>
        ...
        with st.expander("⚙️ 終端設定面板", expanded=True):
            ...
            with m2:
                ...
                if st.session_state.user == "okdycrreoo":
                    ...
                    new_tw = st.number_input("趨勢增益", 0.5, 3.0, tw_val)
                    new_ttl = st.number_input("API 分鐘", 1, 10, api_ttl)
                    ...
                    new_v = st.slider(f"波動補償 (AI建議: {rec_v})", 0.1, 3.0, v_comp)
