...
def chapter_7_ai_committee_analysis(symbol, brain_row):
    ...
    if st.button(f"🚀 啟動 {pure_code} 專業流程分析", key=f"ai_v34_{pure_code}", type="primary", use_container_width=True):
        ...
        c_name = info["name"]
        if not c_name:
            st.error(f"❌ 驗證失敗：代號 {pure_code} 無法於任何市場查獲。")
            return
        status.success(f"✅ 確認公司：{c_name} | 官方分類：{info['industry']}")

        try:
            # 流程 2 & 3: 業務、供應鏈與深度新聞 (改良版搜尋字串)
            status.info(f"Step 2 & 3: 深度挖掘「{c_name}」業務與上下游供應鏈...")
            context_data = ""
            with DDGS() as ddgs:
                # 擴大搜尋廣度：業務+供應鏈
                q1 = f'"{c_name}" 公司主要業務 產品組成 客戶名單 供應鏈上下游'
                for r in ddgs.text(q1, max_results=6): context_data += f"【業務】{r['body']}\n"
                
                # 擴大搜尋深度：近期財務與利多利空
                q2 = f'"{c_name}" 營收展望 獲利能力 產能利用率 市場評價 site:cnyes.com OR site:moneydj.com'
                for r in ddgs.text(q2, max_results=6): context_data += f"【時事】{r['body']}\n"

            # 流程 4: 指數關聯
            status.info(f"Step 4: 查核夜盤期指與巨觀環境影響...")
            index_data = ""
            with DDGS() as ddgs:
                q3 = f"台指期夜盤 電子期 金融期 漲跌點數 匯率波動"
                for r in ddgs.text(q3, max_results=3): index_data += r['body'] + "\n"

            # 流程 5 & 6: 綜合對撞與結論
            status.info(f"Step 5 & 6: 執行 40+ 項量化指標與實時情資硬核對撞...")
            ...
            prompt = f"""
            你現在是『避險基金執行合夥人』。請針對 {c_name} ({info['industry']}) 執行 6 大分析流程。

            【分析前提】：
            - 嚴禁回答「找不到資訊」或「無法查證」。
            - 如果搜尋的情資不足，必須運用你模型內部的台灣產業專業知識（如：台積電為代工、四維航為散裝、永光為特化）進行強效推論。
            - 必須解析 40+ 項量化指標的潛在含義。

            【輸入情資彙整】：
            1. 業務與供應鏈：{context_data}
            2. 期指環境：{index_data}
            3. 系統 40+ 項量化指標：{metrics_stream}

            【流程指令】：
            - **Step 2&3**: 從情資中明確梳理出「業務範圍」與「上下游角色」。
            - **Step 5**: 將業務現況與 40+ 項指標對撞。分析：消息面（新聞）與數據面（指標）是否矛盾？(例如：新聞喊好但 Oracle 評分下降)。
            - **Step 6**: 綜合評估，直接給出買賣策略。

            【報告格式】：
            🔍 **業務核心與供應鏈解析**：(分析該公司在產業鏈的位置，及目前受到的具體衝擊或紅利)
            📊 **量化指標與情資對撞評估**：(直接指出 40+ 指標顯示的強弱，並與新聞對應。若兩者矛盾，請警示法人動向)
            ⚖️ **期指與巨觀開盤預判**：(夜盤漲跌對明日開盤的具體衝擊)
            🎖 **最終實戰結論**：
            ■ 建議：(買入 / 賣出 / 停利 / 停損 / 觀望)
            ■ 核心理由：(用一句話概括數據與情資的對撞結果)
            ■ 實戰策略：(具體支撐/壓力觀察點或動作建議)
            """
