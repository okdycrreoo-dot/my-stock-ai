...
# --- 4. AI 核心：深度微調連動引擎 (已優化：誤差反饋與全自主導航) ---
...
def auto_fine_tune_engine(df, base_p, base_tw, v_comp):
    ...
    # 趨勢權重自動化：環境恐慌時降低信任度，量能噴發時增加權重
    tw_adj = 0.8 if env_panic > 1.0 else 1.0
    f_tw = max(0.5, min(2.2, 1.0 + (rets.tail(5).mean() * 15 * min(1.5, vol_spike)) * tw_adj))
    ...
    # 靈敏度全自主優化：高波動時降低靈敏度以防洗盤
    if env_panic > 1.1:
        f_p = 45  # 防禦模式
    else:
        f_p = (50 if f_vol > 0.02 else 80 if f_vol < 0.008 else 65)
    ...
    return int(f_p), round(float(f_tw), 2), f_v, benchmarks, bias_val, f_vol

def perform_ai_engine(df, p_days, precision, trend_weight, v_comp, bias, f_vol, error_offset=0):
    """
    核心模擬引擎：注入 error_offset 反饋機制進行路徑修正。
    """
    last = df.iloc[-1]
    prev = df.iloc[-2]
    ...
    # 3. 蒙特卡羅路徑模擬 (關鍵優化：納入誤差修正項)
    np.random.seed(42)
    sim_results = []
    # base_drift 方向修正：誤差為正表示過去預測太高，故減去 error_offset
    base_drift = ((int(precision) - 55) / 1000) * float(trend_weight) + (rsi_div * 0.002) + (chip_mom * 0.15) - (error_offset * 0.12)
...
def render_terminal(symbol, p_days, cp, tw_val, api_ttl, v_comp, ws_p):
    ...
    # 1. [新增] 獲取過去誤差數據進行自我修正
    try:
        # 讀取 predictions 工作表，篩選此股票且已有實際收盤價的最近10筆資料
        recs = ws_p.get_all_records()
        df_p = pd.DataFrame(recs)
    ...
    # 2. 執行 AI 引擎 (帶入自動優化的參數與誤差補償 error_offset)
    final_p, final_tw, ai_v, _, bias, f_vol = auto_fine_tune_engine(df, cp, tw_val, v_comp)
    # 注意：這裡將 error_offset 傳入 perform_ai_engine
    ...
    if abs(error_offset) > 0.01:
        st.toast(f"🤖 AI 自我檢討：偵測到近期預測偏{'高' if error_offset > 0 else '低'}，已自動修正偏移量。")
    ...
    # 5. 標題與 Metrics
    st.title(f"📊 {f_id} AI 自主觀察室")
    st.caption(f"✨ 目前狀態：AI 已接管決策係數 | 誤差反饋補償：{error_offset:+.2%} | 數據刷新：{api_ttl}分鐘")
    ...
    # 7. 圖表區 (維持原版設定)
    # [此處 Plotly 圖表代碼與您提供的原版一致，僅需確保使用新的 pred_line 變數即可]
    # ... (省略中間重複的 Plotly 代碼以節省篇幅，請保留您原有的繪圖部分) ...
    ...
    st.markdown(f"""
        <div class='ai-advice-box'>
            ...
            <div style='background: #1C2128; padding: 12px; border-radius: 8px;'>
                <p style='color:#00F5FF; font-weight:bold; margin:0;'>🔮 AI 戰情展望 (誤差補償後)：</p>
...
def main():
    ...
    if st.session_state.user is None:
        ...
        # 1. 頁面最上方導航列
        st.title("🛡️ AI 自主觀察戰情室")
        
        # 2. 頂部縮放按鈕 (原本的設計)
        with st.expander("⚙️ 終端管理與 AI 參數建議 (點擊縮放)", expanded=False):
            m1, m2 = st.columns(2)
            
            with m1:
                st.markdown("#### 📝 自選股清單")
                target = st.selectbox("切換目標", u_stocks if u_stocks else ["2330.TW"])
                p_days = st.number_input("預測天數", 1, 30, 7)
                ns = st.text_input("➕ 新增股票代號")
                c1, c2 = st.columns(2)
                with c1:
                    if st.button("確認新增"):
                        if ns: ws_w.append_row([st.session_state.user, ns.upper().strip()]); st.rerun()
                with c2:
                    if st.button("🗑️ 刪除目前"):
            ...
            with m2:
                if st.session_state.user == "okdycrreoo":
                    st.markdown("#### 🛠️ AI 學習參數手動設定")
                    # 在此先呼叫 AI 引擎算出「建議值」
                    temp_df, _ = fetch_comprehensive_data(target, api_ttl*60)
                    if temp_df is not None:
                        ai_p, ai_tw, ai_v, ai_b, _, _ = auto_fine_tune_engine(temp_df, cp, tw_val, v_comp)
                        
                        # 手動輸入欄位，旁邊顯示 AI 推薦
                        b1 = st.text_input(f"權值標本 (AI推薦: {ai_b[0]})", s_map.get('benchmark_1', "2330.TW"))
                        b2 = st.text_input(f"成長標本 (AI推薦: {ai_b[1]})", s_map.get('benchmark_2', "2382.TW"))
                        b3 = st.text_input(f"指數標本 (AI推薦: {ai_b[2]})", s_map.get('benchmark_3', "00878.TW"))
                        
                        new_p = st.slider(f"系統靈敏度 (AI推薦: {ai_p})", 30, 95, cp)
                        new_tw = st.number_input(f"趨勢權重 (AI推薦: {ai_tw})", 0.5, 3.0, tw_val)
                        new_v = st.slider(f"波動補償 (AI推薦: {ai_v})", 0.5, 3.0, v_comp)
                        new_ttl = st.number_input("API 連線時間 (1~10分鐘)", 1, 10, api_ttl)
                        
                        if st.button("💾 儲存手動設定至雲端"):
