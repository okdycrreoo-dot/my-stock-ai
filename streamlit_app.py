...
st.markdown("""
    ...
    div[data-baseweb="select"] > div { 
        background-color: #1C2128 !important; 
        color: #FFFFFF !important; /* 選取後的文字強制白色 */
        border: 2px solid #00F5FF !important; 
    ...
    /* 5. 修正滑桿 (Slider) 文字顏色 (靈敏度、快取時間) */
    .stSlider [data-testid="stWidgetLabel"] p { color: #00F5FF !important; font-size: 1.1rem !important; }
    .stSlider [data-testid="stTickBarMin"], .stSlider [data-testid="stTickBarMax"] { color: #FFFFFF !important; }

    /* 6. 青色按鈕設計 (按鈕文字改深色以利對比) */
    .stButton>button { 
...
def show_ultimate_dashboard(symbol, unit, p_days, precision):
    ...
    if df is None:
        st.error(f"❌ 無法讀取股票代碼 '{symbol}'。請檢查代碼是否正確。")
        return
        ...
        with st.expander("⚙️ 終端功能面板 (管理清單與系統參數)", expanded=False):
                ...
                if user_stocks and st.button(f"🗑️ 刪除 {target}"):
                    ...
                    for i, row in enumerate(rows):
                        if i > 0 and row[0] == st.session_state.user and row[1] == target:
                            ws_watch.delete_rows(i + 1)
                            st.rerun()
                
                ns = st.text_input("➕ 新增代碼 (例: 2454.TW)", key="add_stock")
                if st.button("確認新增股票"):
                    if ns and ns.upper() not in user_stocks:
                        ws_watch.append_row([st.session_state.user, ns.upper()])
                        st.rerun()
            ...
            with m2:
                ...
                if st.session_state.user == "okdycrreoo":
                    ...
                    st.info(f"靈敏度目前由管理員設定為: {curr_prec}")
