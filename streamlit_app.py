...
def render_terminal(symbol, p_days, cp, tw_val, api_ttl, v_comp, ws_p):
    ...
    st.markdown("""
        ...
        .info-box { background: #0A0A0A; padding: 12px; border: 1px solid #333; text-align: center; border-radius: 10px; }
    ...
    # [Metrics 與 建議價格區間 代碼維持不變...]

    # 2. 圖表配置：將線標說明直接嵌入 subplot_titles
    # 這裡我們利用 HTML 標籤直接在標題後面加上線標說明，解決座標不移動的問題
    title_main = "■ 價格與均線 <span style='color:#AAA; font-size:14px; font-weight:normal;'> &nbsp;&nbsp; <span style='color:#FF4444'>●</span> K線 <span style='color:#FFEE58'>—</span> 5MA <span style='color:#18FFFF'>—</span> 10MA <span style='color:#F06292'>—</span> 20MA <span style='color:#FF1744'>···</span> AI預測</span>"
    title_vol  = "■ 成交量 (張)"
    title_macd = "■ MACD 指標 <span style='color:#AAA; font-size:14px; font-weight:normal;'> &nbsp;&nbsp; <span style='color:#FF5252'>■</span> 能量柱 <span style='color:#FFFFFF'>—</span> DIF <span style='color:#FFA726'>—</span> DEA</span>"
    title_kdj  = "■ KDJ 擺動指標 <span style='color:#AAA; font-size:14px; font-weight:normal;'> &nbsp;&nbsp; <span style='color:#18FFFF'>—</span> K值 <span style='color:#FFFF00'>—</span> D值 <span style='color:#E066FF'>—</span> J值</span>"

    fig = make_subplots(
        rows=4, cols=1, shared_xaxes=True, 
        row_heights=[0.4, 0.15, 0.2, 0.25], vertical_spacing=0.07,
        subplot_titles=(title_main, title_vol, title_macd, title_kdj)
    )
    
    p_df = df.tail(90)
    # [繪圖 Trace 部分：確保 showlegend=False 避免原生圖例干擾...]
    fig.add_trace(go.Candlestick(x=p_df.index, open=p_df['Open'], high=p_df['High'], low=p_df['Low'], close=p_df['Close'], increasing_line_color='#FF4444', decreasing_line_color='#00FF41', showlegend=False), 1, 1)
    # ... (其餘 Scatter 與 Bar 代碼同前，請確保 showlegend=False)
    ...
    # 重要：由於我們使用了嵌入式 HTML 標題，需要確保 Plotly 允許 HTML 渲染並將標題靠左
    for i in fig['layout']['annotations']:
        i['x'] = 0                # 標題起點靠最左
        i['xanchor'] = 'left'     # 錨點在左
        i['font'] = dict(size=14, color="#FFFFFF") # 標題基礎大小
        i['align'] = 'left'
