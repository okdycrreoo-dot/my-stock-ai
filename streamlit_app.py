...
def get_user_data(conn):
    try:
        # 讀取第一個工作表，ttl=0 確保每次讀取最新數據
        df = conn.read(ttl=0)
        # 確保有 username 這一欄，否則回傳空表
...
if not st.session_state['logged_in']:
    ...
    # 使用 key 參數防止 DuplicateElementId 錯誤
    ...
    if mode == "註冊帳號":
        if st.sidebar.button("確認註冊並存入雲端", key="reg_btn"):
            if u and p and u not in df_users["username"].astype(str).values:
                ...
                conn.update(data=updated_df) # 寫回 Google Sheets
