...
def chapter_7_ai_committee_analysis(symbol, brain_row):
    ...
    if st.button(f"🚀 啟動 {pure_code} 全球情資對撞分析", key=f"ai_v34_{pure_code}", type="primary", use_container_width=True):
        ...
        try:
            ...
            # --- 步驟 4: AI 深度對撞分析 (修正：鎖定官方產業別與數據標籤) ---
            ...
            prompt = f"""
            你現在是嚴謹的『首席法人稽核策略長』。請針對 {c_name} 進行全維度事實對撞。

            【官方認證背景】：
            - 官方產業類別：{c_industry} (由 TWSE 證交所定義)
            - 核心量化指標：{metrics_summary} 
            (註：指標內含該公司最新 EPS、毛利率、本益比等重要財務數據，請務必解析並引用)

            【分析鐵律】：
            1. **產業定位**：必須以官方類別「{c_industry}」為分析基石。嚴禁說出業務不明。例如永光為化學工業，應聚焦特化與電子化學；四維航為航運業，應聚焦散裝航運與 BDI 指數。
            2. **數據獲利對撞**：
               - 強制引用【核心量化指標】中的 EPS 與毛利率進行基本面分析。
               - 結合新聞「{local_news}」查核近期動態。
               - 即使 2026 新聞數字較少，也必須根據量化指標中的歷史獲利能力給出結論，禁止回答「無數據」。
            3. **夜盤影響**：根據「{global_env}」預判明日對該產業的開盤衝擊。

            【報告格式 (務必換行兩次)】：
            🔍 **{c_name} ({c_industry}) 財務事實查核**：(請直接列出底層數據中的 EPS、毛利率表現，並結合官方產業定位說明)

            📊 **數據與時事矛盾對沖**：(分析量化指標與最新消息是否同步。若數據強勁但新聞偏空，請指出矛盾點)

            ⚖️ **夜盤環境與匯率衝擊**：(根據夜盤與匯率推算對明日開盤的實質影響)
            ...
            ■ 盯盤重點：(明天需關注的指標，如：BDI、油價、匯率或特定板塊)
            """
