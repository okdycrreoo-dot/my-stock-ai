...
def chapter_7_ai_committee_analysis(symbol, brain_row):
    ...
    # --- 步驟 1: 官方全市場正名 (修正快取 key，確保換代號會更新) ---
    @st.cache_data(ttl=3600) # 縮短至一小時，且 code 作為參數確保唯一性
    def get_verified_name_dynamic(code):
        # A. 證交所
        try:
    ...
    # 這裡的 Button Key 加入 pure_code 確保切換股票時按鈕狀態會重置
    if st.button(f"🚀 啟動 {pure_code} 全球情資對撞分析", key=f"ai_v34_{pure_code}", type="primary", use_container_width=True):
        ...
        c_name = get_verified_name_dynamic(pure_code)
        ...
        try:
            ...
            # --- 步驟 4: 最終手術修正：AI 全維度事實對撞 (含歷史財報與技術數據) ---
            status.info(f"⚖️ AI 大腦正在將「歷史財報」與「即時時事」進行對撞...")
            ...
            prompt = f"""
            你現在是嚴謹的『法人稽核策略長』。請針對 {c_name} 進行全維度事實對撞。

            【系統底層財報與量化數據】：
            - 核心指標數據：{metrics_summary} 
            (註：此數據包含該公司最新公布的 EPS、毛利率、本益比及 40 項技術指標，請優先作為基本面判斷依據)

            【分析鐵律 - 拒絕空話與幻覺】：
            1. **產業別查核**：首先確認 {c_name} 的核心業務。如果是航運，嚴禁提到 AI 或半導體題材；若是電子，則需對應供應鏈。
            2. **多維度獲利對撞**：
               - 必須引用【系統底層數據】中的 EPS 與毛利率，分析其歷史獲利穩定性。
               - 對照最新新聞「{local_news}」查核近期業績動態。
               - 若 2026 新聞無數字，請根據底層數據中的第三季/年報數值進行分析，嚴禁回答「無數據」。
            3. **量化對沖**：結合指標數據 [{metrics_summary}]，分析目前股價相對於基本面（EPS）是低估還是過熱。
            4. **環境預測**：根據「{global_env}」預估明日開盤衝擊。

            【報告格式 (務必換行兩次)】：
            🔍 **{c_name} 財務基本面與產業事實**：(請具體列出底層數據中的 EPS、毛利率等關鍵表現，並查核其產業地位)

            📊 **量化指標與時事矛盾對沖**：(分析底層量化指標與最新消息是否同步。若數據強勁但新聞偏空，請指出矛盾點)

            ⚖️ **夜盤環境與匯率衝擊**：(根據夜盤與匯率推算對該股明日的實質影響)
            ...
            ■ 盯盤重點：(明天需關注的具體指標，如：商品報價、匯率或特定的市場消息)
            """

            payload = {
                "model": "llama-3.3-70b-versatile",
                "messages": [{"role": "user", "content": prompt}],
                "temperature": 0.0, # 保持零隨機性，確保邏輯一致
                "max_tokens": 1500
            }
